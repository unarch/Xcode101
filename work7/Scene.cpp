...
//    if ((inter_l.coords - inter_tmp.coords).norm() < EPSILON) {
//        L_dir = inter_l.emit * p.m->eval(wo, wi.direction, p.normal) *
//        dotProduct(wi.direction, p.normal) * dotProduct(-wi.direction, inter_l.normal) / (p2lightDist * p2lightDist) / pdf;
//
//    }
//
...
//
//    if (q.happened and !q.obj->hasEmit()) {
